  ...
  router.post('/:id/approve', async (req, res) => {
    ...
    try {
      ...
      if (existingUsers.length > 0) {
        // User record already exists, just link and approve
        createdUserId = existingUsers[0].id;
      } else {
        // 3. Resolve role_id
        let role_id = 13; // General Employee role fallback
        const targetRoleName = request.role_name || request.role_type || 'Employee';
        ...
        // 5. Insert new user into users table with status 'Active'
        const [insertResult] = await connection.query(
          `INSERT INTO users (
            first_name, last_name, email, username, password, phone1, location, role_id, status, department, job_title
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'Active', ?, ?)`,
          [
            request.first_name,
            request.last_name || '',
    ...
    try {
      ...
      await connection.query(
        `UPDATE registration_requests 
         ...
         WHERE id = ?`,
        [reviewed_by || null, reason || null, id]
      );
